...
function mergeAllowedTools(tools) {
    const merged = new Set();
    for (const toolName of tools ?? [])
        merged.add(toolName);
    for (const toolName of HAL_AUTO_ALLOWED_TOOLS)
        merged.add(toolName);
...
async function callHalTool(toolName, args, context) {
    ...
    if (cmd.success === true) {
        ...
        const structured = typeof result === 'object' && result !== null ? result : { result };
        return {
    ...
    const structured = typeof error === 'object' && error !== null ? error : { error };
    return {
...
function buildHalMcpServer(context) {
    const tools = [
        tool('hal_memory_search', 'Search HAL semantic memory (vector recall).', {
            query: z.string(),
            source: z.string().optional(),
            limit: z.number().int().positive().optional(),
            threshold: z.number().min(0).max(1).optional()
        }, async (args) => callHalTool('hal_memory_search', args, context)),
        tool('hal_memory_store', 'Store information in HAL semantic memory.', {
            content: z.string(),
        ...
        tool('hal_memory_forget', 'Delete a memory from HAL semantic memory by id.', {
            memory_id: z.string()
        }, async (args) => callHalTool('hal_memory_forget', args, context)),
        tool('hal_calendar_get_events', 'List upcoming calendar events.', {
            start_date: z.string().optional(),
            end_date: z.string().optional(),
            limit: z.number().int().positive().optional()
        }, async (args) => callHalTool('hal_calendar_get_events', args, context)),
        tool('hal_calendar_create_event', 'Create a calendar event.', {
            title: z.string(),
            start_time: z.string(),
            end_time: z.string(),
        ...
        tool('hal_tasks_list', 'List tasks (Linear).', {
            status: z.string().optional(),
            limit: z.number().int().positive().optional(),
            project: z.string().optional()
        }, async (args) => callHalTool('hal_tasks_list', args, context)),
        tool('hal_tasks_create', 'Create a task (Linear).', {
            title: z.string(),
            description: z.string().optional(),
            priority: z.enum(['low', 'medium', 'high', 'urgent']).optional(),
            team_id: z.string()
        }, async (args) => callHalTool('hal_tasks_create', args, context)),
        tool('hal_tasks_complete', 'Mark a task complete (Linear).', {
            task_id: z.string()
        }, async (args) => callHalTool('hal_tasks_complete', args, context)),
        tool('hal_send_notification', 'Send a notification to the user.', {
            message: z.string(),
            channel: z.enum(['auto', 'telegram', 'email']).optional(),
            priority: z.enum(['low', 'normal', 'high', 'urgent']).optional(),
            silent: z.boolean().optional()
        }, async (args) => callHalTool('hal_send_notification', args, context)),
        tool('hal_delegate_to_codex', 'Delegate a complex coding task to OpenAI Codex. If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            task: z.string(),
            project_path: z.string().optional(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_delegate_to_codex', args, context)),
        tool('hal_delegate_to_jules', 'Delegate a long-running background task to Jules. If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            task: z.string(),
            repo: z.string().optional(),
            priority: z.enum(['low', 'normal', 'high']).optional(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_delegate_to_jules', args, context)),
        tool('hal_delegate_to_gemini', 'Delegate a summarization or completion task to Gemini.', {
            text: z.string(),
        ...
        tool('hal_check_delegation_status', 'Check the status of a delegated task (Codex, Jules, or Gemini).', {
            task_id: z.string(),
            agent: z.enum(['codex', 'jules', 'gemini'])
        }, async (args) => callHalTool('hal_check_delegation_status', args, context)),
        tool('hal_codops_create_change_request', 'Create a codops ChangeRequest (isolated worktree → diff → tests). If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            task: z.string(),
            title: z.string().optional(),
            test_command: z.string().optional(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_codops_create_change_request', args, context)),
        tool('hal_codops_apply_change_request', 'Apply a ready ChangeRequest patch to the main repo. If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            change_request_id: z.string(),
            force: z.boolean().optional(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_codops_apply_change_request', args, context)),
        tool('hal_mcp_start_named_connection', 'Start a named MCP server connection from ~/.hal/mcp-servers.json. If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            name: z.string(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_mcp_start_named_connection', args, context)),
        tool('hal_mcp_list_connections', 'List active MCP connections.', {}, async (args) => callHalTool('hal_mcp_list_connections', args, context)),
        tool('hal_mcp_list_tools', 'List tools for a named MCP connection.', {
            name: z.string()
        }, async (args) => callHalTool('hal_mcp_list_tools', args, context)),
        tool('hal_mcp_call_tool', 'Call a tool on a named MCP connection. If approval_token is omitted, HAL will create an approval request and return a token to approve in /approvals.', {
            name: z.string(),
            tool: z.string(),
            arguments: z.record(z.string(), z.any()).optional(),
            approval_token: z.string().optional()
        }, async (args) => callHalTool('hal_mcp_call_tool', args, context)),
        tool('hal_browser_navigate', 'Navigate to a URL in the browser (devtools or extension).', {
            url: z.string(),
            mode: z.enum(['devtools', 'extension']).optional(),
            wait_for: z.string().optional()
        }, async (args) => callHalTool('hal_browser_navigate', args, context)),
        tool('hal_browser_snapshot', 'Take an accessibility tree snapshot of the current page (devtools).', {
            verbose: z.boolean().optional()
        }, async (args) => callHalTool('hal_browser_snapshot', args, context)),
        tool('hal_browser_click', 'Click an element (by uid or selector).', {
            uid: z.string().optional(),
            selector: z.string().optional(),
            double_click: z.boolean().optional()
        }, async (args) => callHalTool('hal_browser_click', args, context)),
        tool('hal_browser_fill', 'Fill a form field (by uid or selector).', {
            uid: z.string().optional(),
            selector: z.string().optional(),
            value: z.string()
        }, async (args) => callHalTool('hal_browser_fill', args, context)),
        tool('hal_browser_screenshot', 'Take a screenshot (devtools).', {
            full_page: z.boolean().optional(),
            selector: z.string().optional()
        }, async (args) => callHalTool('hal_browser_screenshot', args, context)),
        tool('hal_browser_evaluate', 'Evaluate JavaScript (devtools).', {
            script: z.string()
        }, async (args) => callHalTool('hal_browser_evaluate', args, context)),
        tool('hal_browser_console', 'List browser console messages (devtools).', {
            level: z.string().optional(),
            limit: z.number().int().positive().optional()
        }, async (args) => callHalTool('hal_browser_console', args, context)),
        tool('hal_browser_network', 'List browser network requests (devtools).', {
            filter: z.string().optional(),
            type: z.string().optional(),
            limit: z.number().int().positive().optional()
        }, async (args) => callHalTool('hal_browser_network', args, context)),
        tool('hal_self_update_personality', 'Update a personality trait (rate-limited).', {
            trait: z.string(),
            new_value: z.number().min(0).max(1),
            reason: z.string()
        }, async (args) => callHalTool('hal_self_update_personality', args, context)),
        tool('hal_self_update_preference', 'Store a learned user preference.', {
            key: z.string(),
            value: z.any()
        }, async (args) => callHalTool('hal_self_update_preference', args, context)),
        tool('hal_self_set_goal', 'Create a new goal.', {
            title: z.string(),
            description: z.string().optional(),
        ...
        tool('hal_self_update_goal', 'Update goal progress or status.', {
            goal_id: z.string(),
            progress: z.number().min(0).max(1).optional(),
            status: z.enum(['active', 'paused', 'completed', 'abandoned']).optional(),
            summary: z.string().optional()
        }, async (args) => callHalTool('hal_self_update_goal', args, context)),
        tool('hal_self_reflect', 'Log a reflection/learning for self-improvement.', {
            type: z.string().optional(),
            observation: z.string(),
            impact: z.string().optional(),
            confidence: z.enum(['low', 'medium', 'high']).optional()
        }, async (args) => callHalTool('hal_self_reflect', args, context)),
        tool('hal_get_self_status', 'Get current self status (personality, goals, observations).', {
        ...
        tool('hal_skill_run', 'Run a configured skill from .claude/skills through HAL (skills-first execution boundary).', {
            skill: z.string(),
            input: z.string(),
...
async function handleCreateSession(cmd) {
    const { id } = cmd;
...
async function handleCloseSession(cmd) {
