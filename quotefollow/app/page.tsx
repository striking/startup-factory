                ...
                <div className="flex flex-col md:flex-row md:items-stretch gap-4">
                  {[
                    { 
                      step: 1, 
                      title: 'Quote sent', 
                      description: '"Bathroom reno — $8,500"', 
                      containerClasses: 'flex-1 bg-slate-900/40 border border-slate-700/50 rounded-lg p-4', 
                      stepTextClasses: 'text-xs text-slate-400 mb-2', 
                      titleClasses: 'font-medium' 
                    },
                    { 
                      step: 2, 
                      title: 'Auto SMS + email follow-up', 
                      description: '"Just checking you received the quote"', 
                      containerClasses: 'flex-1 bg-blue-900/20 border border-blue-500/20 rounded-lg p-4', 
                      stepTextClasses: 'text-xs text-blue-300/80 mb-2', 
                      titleClasses: 'font-medium text-blue-200' 
                    },
                    { 
                      step: 3, 
                      title: 'Customer replies', 
                      description: '"Yep — can you start next week?"', 
                      containerClasses: 'flex-1 bg-slate-900/40 border border-slate-700/50 rounded-lg p-4', 
                      stepTextClasses: 'text-xs text-slate-400 mb-2', 
                      titleClasses: 'font-medium' 
                    },
                    { 
                      step: 4, 
                      title: 'Hot quote alert', 
                      description: "You're notified instantly to close the job.", 
                      containerClasses: 'flex-1 bg-green-900/20 border border-green-500/20 rounded-lg p-4', 
                      stepTextClasses: 'text-xs text-green-300/80 mb-2', 
                      titleClasses: 'font-medium text-green-200' 
                    },
                  ].map((s, i, arr) => (
                    <React.Fragment key={s.step}>
                      <div className={s.containerClasses}>
                        <div className={s.stepTextClasses}>Step {s.step}</div>
                        <div className={s.titleClasses}>{s.title}</div>
                        <div className="mt-2 text-sm text-slate-300">{s.description}</div>
                      </div>
                      {i < arr.length - 1 && <div className="hidden md:flex items-center text-slate-600">→</div>}
                    </React.Fragment>
                  ))}
                </div>
              </div>
